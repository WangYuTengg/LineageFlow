...
class Test(APIView):
    def post(self, request):
        ...
        print(
            f"Repo: {repo_name}, Branch: {branch_name}, Files: {len(files)}, Relative Paths: {len(relative_paths)}, Storage Bucket URL: {storage_bucket_url}, commit_message: {commit_message} "
        )
    ...
    def handle_empty_branch(
        ...
        with transaction.atomic():
            list_of_file_obj = self.create_file_objects(
                files, relative_paths, repo_name, branch_name, storage_bucket_url, gcs
            )
            ...
            self.create_metarange_and_commit(
                ...
                list_of_file_obj_add=list_of_file_obj,
                commit_message=commit_message,
            )

    def handle_existing_branch(
        self,
        files,
        relative_paths,
        repo_name,
        ...
        for file, relative_path in zip(files, relative_paths):
            ...
            existing_file = (
                File.objects.filter(file_name=file_name).order_by("-version").first()
            )

            if not existing_file:
                # Upload and create file for the first time
                print(f"Uploading and creating new file: {file_name}")
                new_file_obj = self.upload_and_create_file(
                    file, relative_path, repo_name, branch_name, storage_bucket_url, gcs
                )
        ...
        new_file_partitions = self.partition_files_by_size(files_first_time_added)
        list_of_range_of_new_files = self.create_ranges_and_partitions(new_file_partitions)
        all_existing_ranges.extend(list_of_range_of_new_files)
        ...
        self.create_metarange_and_commit(
            branch,
            all_existing_ranges,
            list_of_file_obj_add=files_first_time_added,
            list_of_file_edit=files_edited,
            commit_message=commit_message,
        )
    ...
    def upload_and_create_file(
        ...
        file_name = f"{repo_name}/{branch_name}/{relative_path}"
        obj_data = gcs.upload_and_get_metadata(
            file, relative_path, storage_bucket_url, version
        )
        print(obj_data)
        loc = obj_data["url"]
        meta_data = obj_data["meta_data"]
        new_file = File.objects.create(
            file_name=file_name, loc=loc, meta_data=meta_data, version=version
        )
        print(f"Created file object: {file_name}, Version: {version}")
        return new_file

    def create_file_objects(
        self, files, relative_paths, repo_name, branch_name, storage_bucket_url, gcs
    ):
    ...
    def create_ranges_and_partitions(self, partitions):
        ...
        for partition in partitions:
            partition_size = sum(
                int(json.loads(file.meta_data)["size"]) for file in partition
            )  # Calculate total partition size
            range_obj = Range.objects.create(
                size=partition_size
            )  # Create Range with partition size
    ...
    def create_metarange_and_commit(
        ...
        branch,
        list_of_range_objects,
        list_of_file_obj_add=[],
        list_of_file_edit=[],
        commit_message="Auto-generated message",
    ):
        print("Creating MetaRange and Commit")
        ...
        new_commit = Commit.objects.create(
            branch=branch,
            commit_message=commit_message,  # Add a default message or adjust as necessary
        )
